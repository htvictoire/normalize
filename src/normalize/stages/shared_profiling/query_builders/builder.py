...
def build_profile_query(
    ...
    *,
    ...
    for index, column_name in enumerate(columns):
        ...
        source_exprs.append(
            f"{apply_accounting_sign_expr(currency_symbol_stripped_alias)} "
...
def build_pass2_currency_query(
    columns: list[str],
    *,
    table_name: str,
    decimal_separator: str,
    ...
    Runs a second table scan but only for columns that pass1 showed are not
    already clearly decimal/integer/boolean.

    Pass1 already measured plain-decimal matches. Pass2 only computes the
    currency-specific increment (token/accounting marked values), which avoids
    repeating base decimal regex and TRY_CAST work.
    """
    source_exprs: list[str] = []
    exprs: list[str] = []
    decimal = decimal_pattern(
    ...
    for index, column_name in enumerate(columns):
        ...
        currency_extra_match_predicate = (
            f"({marker_predicate}) AND {currency_value_match_predicate}"
        )
        exprs.append(
            "SUM(CASE "
            f"WHEN {base_value} IS NULL THEN 0 "
            f"WHEN {currency_extra_match_predicate} THEN 1 "
            "ELSE 0 END) "
            f"AS {column_name}__currency_extra_match_count"
        )
        exprs.append(
            "SUM(CASE "
            f"WHEN {base_value} IS NULL THEN 0 "
